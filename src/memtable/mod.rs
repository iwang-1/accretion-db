//! The in-memory write buffer: a sorted `BTreeMap` behind an `RwLock`, with a size-triggered *atomic
//! freeze* that keeps concurrent reads correct while a frozen buffer is being flushed to an SSTable. # Shape *.
...
/// What a versioned value *is*: either a live value or a deletion marker. Tombstones are first-class so
/// that a delete can shadow an older value that still lives in a lower tier.
...
/// A value tagged with the sequence number of the write that produced it. Ordering between two
/// `InternalValue`s for the *same key* is by [`seq`](Self::seq): the larger sequence number is newer and wins.
...
pub struct Memtable {
    ...
    /// Approximate live byte count: sum over entries of key + payload + a fixed per-entry overhead. Used
    /// only to decide when to freeze, so an estimate is fine.
    approx_bytes: usize,
}
...
impl Memtable {
    ...
    /// Insert `key` only if `val` is at least as new as any value already present (by
    /// [`InternalValue::seq`]); a stale write is dropped.
...
/// The engine's in-memory buffer: one mutable *active* table plus a list of *frozen* tables awaiting flush,
/// all behind a single `RwLock`.
#[derive(Debug)]
...
impl MemtableSet {
    ...
    /// Insert `key` into the active table only if `val` is newer (by seq) than the value currently there.
    pub fn insert_if_newer(&self, key: Vec<u8>, val: InternalValue) {
        let mut st = self.state.write().expect("memtable lock poisoned");
        st.active.insert_if_newer(key, val);
    }

    /// Resolve `key` to its newest version across active + frozen tables, or `None` if no table mentions
    /// it.
    pub fn get(&self, key: &[u8]) -> Option<InternalValue> {
    ...
    /// Atomically move the active table onto the frozen list and install a fresh empty active table;
    /// returns an `Arc` to the just-frozen table.
    ...
    /// Build one boxed entry iterator per in-memory table (active + every frozen table), each pre-filtered
    /// to `range`, ready to feed the merge iterator.

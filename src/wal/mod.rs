//! Write-ahead log: crc-framed, segmented, with a mode-driven commit pipeline. The WAL is the engine's
//! durability anchor.
...
/// The segment-writing half of the WAL, guarded by its own mutex so the group-commit leader can hold it
/// across I/O while the coordinator lock is free for other writers to enqueue.
#[derive(Debug)]
struct Writer {
    active: Segment,
}
...
impl Wal {
    ...
    /// Append one record `payload` and return only once the configured [`Durability`] contract is met.
    ...
    fn commit_group(&self, frame: Vec<u8>) -> StorageResult<()> {
        ...
        loop {
            ...
            match &io {
                ...
                Err(_) => {
                    // The batch was not made durable. Return the taken frames to the front of the queue
                    // (they hold the lowest seqs, so this preserves order) so a subsequent leader retries them rather than silently stranding parked followers.
...
mod tests {
    ...
    /// Group commit under real thread concurrency: many writers, every acked record must be present and
